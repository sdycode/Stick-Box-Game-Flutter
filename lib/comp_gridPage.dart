...
class CompGridPage extends StatefulWidget {
  int gridno;
  int p1no;
  int p2no;
  List<Offset> hPoints;
  ...
  double x;
  double s;
  CompGridPage(this.gridno, this.p1no, this.p2no, this.hPoints, this.vPoints,
      this.x, this.s,
      {Key? key})
      : super(key: key);

  @override
  State<CompGridPage> createState() => _CompGridPageState();
}

class _CompGridPageState extends State<CompGridPage>
    with WidgetsBindingObserver {
  late vp p;
  double ratio = Sizes().sh / Sizes().sw;

  int computerplaytimeinmillisec = 800;
  ...
  bool showCompSpeedChange = false;
              ...
              Container(
                ...
                child: Padding(
                  ...
                  child: Row(
                      ...
                      //   onTap: () {
                      //     setState(() {
                      //       showCompSpeedChange = !showCompSpeedChange;
                        ...
                        children: [
                          ...
                          ...(widget.hPoints.map((e) {
                            return Positioned(
                              ...
                              //  widget.x,
                              top:
                                  e.dy + ((w - boxsize) * 0.5 - widget.x).abs(),
                              child: Transform.scale(
                          ...
                          ...(widget.vPoints.map((e) {
                            return Positioned(
                              left:
                                  e.dx + ((w - boxsize) * 0.5 - widget.x).abs(),
                                ...
                                child: Container(
                                  ...
                                  child: GestureDetector(
                                    onTapDown: (d) {
                                      ...
                                      finali = k * n * 2 + 2 * (l) + n;
                                      print(
                                          'valuesss $k / $n / $l /  ${widget.gridno} / $indx  / $finali  / ${!lines[finali]!.marked}');
    ...
    return IgnorePointer(
      ...
      child: Transform.scale(
        ...
        child: Container(
          ...
          height: bh,
          decoration: BoxDecoration(
            color: !isGameOver && lastLineIndex == i ? Colors.blue : color,
  ...
  void updateBox(int i) {
    if (lines[i]!.boxType == BoxType.vline) {
      ...
      if (boxes.containsKey(prevind)) {
        ...
        countsList[prevind] = countsList[prevind]! + 1;
      ...
      if (boxes.containsKey(nextind)) {
        ...
        countsList[nextind] = countsList[nextind]! + 1;
      ...
      if (boxes.containsKey(prevind)) {
        boxes[prevind]!.bottom = true;
        print('boxx top  ${boxes[prevind]!.bottom}');
        countsList[prevind] = countsList[prevind]! + 1;
        if (boxes[prevind]!.left == true &&
            boxes[prevind]!.right == true &&
            boxes[prevind]!.top == true &&
      ...
      if (boxes.containsKey(nextind)) {
        boxes[nextind]!.top = true;
        print('boxx bottom ${boxes[nextind]!.bottom}');
        countsList[nextind] = countsList[nextind]! + 1;
        if (boxes[nextind]!.left == true &&
            boxes[nextind]!.right == true &&
            boxes[nextind]!.top == true &&
    ...
    boxes.entries.toList().forEach((e) {
      print(
          'box c ${e.key} -  ${e.value.top} - ${e.value.bottom} - ${e.value.left}- ${e.value.right}');
    });
    print('countt $countsList');
  }
  ...
  Future checkMoveForComputer() async {
    // Future.delayed
    print("countt  calledd");
    if (countsList.containsValue(3)) {
      print("countt  present");

      // int indexttotap = countsList.keys.firstWhere((e) {
      //   return countsList[e] == 3;
      // });
      List alli = [];
      countsList.entries.forEach((e) {
        if (e.value == 3) {
          alli.add(e.key);
        }
      });
      alli.shuffle();
      int indexttotap = alli.first;

      BoxModel boxind = boxes[indexttotap]!;
      int lineind = indexttotap + count;
      print(
          "countt box mode;  ${boxind.left} / ${boxind.top} / ${boxind.right} / ${boxind.bottom} / ");
      List randomints = [];
      if (boxind.bottom == false) {
        lineind = indexttotap + count;
        randomints.add(lineind);
      }
      if (boxind.top == false) {
        lineind = indexttotap - count;
        randomints.add(lineind);
      }
      if (boxind.left == false) {
        lineind = indexttotap - 1;
        randomints.add(lineind);
      }
      if (boxind.right == false) {
        lineind = indexttotap + 1;
        randomints.add(lineind);
      }
      print('randd 3 before ${randomints}');
      randomints.shuffle();
      print('randd  3 after ${randomints}');
      lineind = randomints.last;
      print("countt  $indexttotap");
      Future.delayed(Duration(milliseconds: computerplaytimeinmillisec)).then(
        (value) async {
          print("countt in future $indexttotap");
          print(
              'countt line ind  ${lineind} ${lines[lineind]} / ${lines[lineind]!.marked}  ');

          if (!lines[lineind]!.marked) {
            setState(() {
              isSound ? audioCache.play('smalltap.mp3') : () {};

              lines[lineind]!.marked = true;
              // !lines[finali]!.marked;
              lines[lineind]!.player = turn1 ? 1 : 2;
              print('box 000000000000000000000000');

              lastLineIndex = lineind;
              isBoxDone = false;

              updateBox(lineind);

              checkGameOver();
              print('countt after diction $countsList');

              if (isBoxDone) {
                isSound ? audioCache.play('boxmarked.mp3') : () {};
              } else {
                turn1 = !turn1;
              }
            });
            if (!turn1 && isBoxDone) {
              await checkMoveForComputer();
            }
          }
        },
      );
    } else if (countsList.containsValue(0)) {
      print("countt  present");
      // int indexttotap = countsList.keys.firstWhere((e) {
      //   return countsList[e] == 0;
      // });
      List alli = [];
      countsList.entries.forEach((e) {
        if (e.value == 0) {
      ...
      print('randd 0 before ${randomints}');
      randomints.shuffle();
      print('randd  0 after ${randomints}');
      lineind = randomints.last;
    ...
    } else if (countsList.containsValue(1)) {
      print("countt  present");
      List alli = [];
      countsList.entries.forEach((e) {
        if (e.value == 1) {
      ...
      print('randd before ${randomints}');
      randomints.shuffle();
      print('randd after ${randomints}');
    ...
    } else if (countsList.containsValue(2)) {
      print("countt  present");
      List alli = [];
      countsList.entries.forEach((e) {
        if (e.value == 2) {
